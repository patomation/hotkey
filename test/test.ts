...
const simulateKeyboardEvent: SimulateKeyboardEvent = (
  ...
  const keyboardEvent = new KeyboardEvent(kind, {
    bubbles: true,
    cancelable: true,
    key,
    shiftKey,
...
const testCases: TestCase[] = [
  ...
  { key: '1', command: 'shift+1', shiftKey: true },
  { key: '2', command: 'shift+2', shiftKey: true },
  { key: '3', command: 'shift+3', shiftKey: true },
  { key: '4', command: 'shift+4', shiftKey: true },
  { key: '5', command: 'shift+5', shiftKey: true },
  { key: '6', command: 'shift+6', shiftKey: true },
  { key: '7', command: 'shift+7', shiftKey: true },
  { key: '8', command: 'shift+8', shiftKey: true },
  { key: '9', command: 'shift+9', shiftKey: true },
  // TODO tab
  // TODO enter?
  // Special characters?
  // Arrows
...
testCases.forEach(
  ({ key, command, ctrlKey = false, altKey = false, shiftKey = false }) => {
    describe(command, (): void => {
      ...
      it('exists in storage', (): void => {
        expect(
          storage[formatCommandString(command)] instanceof HotkeyInstance
        ).to.equal(true)
      })
